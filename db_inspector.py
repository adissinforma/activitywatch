...
class ActivityWatchDBInspector:
    ...
    def print_db_stats(self):
        ...
        try:
            if event_count > 0:
                cursor.execute(f"SELECT {self.event_time_start} FROM {self.event_table} ORDER BY {self.event_time_start} LIMIT 1")
                first_val = cursor.fetchone()
                
                cursor.execute(f"SELECT {self.event_time_start} FROM {self.event_table} ORDER BY {self.event_time_start} DESC LIMIT 1")
                last_val = cursor.fetchone()
                
                if first_val and last_val:
                    first_ts = first_val[0]
                    last_ts = last_val[0]
                    
                    # Intentar interpretar como timestamp numérico o ISO string
                    if isinstance(first_ts, (int, float)):
                        start_dt = datetime.fromtimestamp(first_ts / 1_000_000)
                    else:
                        try:
                            start_dt = datetime.fromisoformat(str(first_ts).replace('Z', '+00:00'))
                        except:
                            start_dt = None
                    
                    if isinstance(last_ts, (int, float)):
                        end_dt = datetime.fromtimestamp(last_ts / 1_000_000)
                    else:
                        try:
                            end_dt = datetime.fromisoformat(str(last_ts).replace('Z', '+00:00'))
                        except:
                            end_dt = None
        except Exception as e:
            print(f"  Nota: Error obteniendo rango de fechas: {e}")
